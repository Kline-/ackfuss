...
bool set_hunt( CHAR_DATA * ch, CHAR_DATA * fch, CHAR_DATA * vch, OBJ_DATA * vobj, int set_flags, int rem_flags )
{
    ...
    snprintf( buf, MSL, "%s has started hunting (%s) %s",
              ch->get_name(),
          ( vch ? IS_NPC( vch ) ? "mobile" : "player" : "object" ), ( vch ? vch->get_name() : CSTR( vobj->short_descr ) ) );
    if ( fch )
...
DO_FUN(do_hunt)
{
    ...
    if ( !IS_NPC( ch ) && IS_WOLF( ch ) && ( IS_SHIFTED( ch ) || IS_RAGED( ch ) ) )
        ...
        victim = ( ch->gTrust() >= MAX_LEVEL ? get_char_world( ch, arg ) : get_char_area( ch, arg ) );
    if ( victim == ch )
    {
        if ( ch->hunting )
        {
    ...
    if ( !IS_NPC( ch ) && IS_WOLF( ch ) && ( IS_SHIFTED( ch ) || IS_RAGED( ch ) ) )
        ...
        ch->hunt_flags = ( ch->gTrust() >= MAX_LEVEL ? HUNT_WORLD : 0 )
